...
```bash
npx semantic-code-search-cli index
npx semantic-code-search-cli ask "where is auth handled?"
```
...
```bash
npx semantic-code-search-cli ask "where are CLI commands registered?" --top-k 3
npx semantic-code-search-cli ask "auth middleware" --json
npx semantic-code-search-cli ask "auth middleware" --no-snippets --top-k 10
```

Local hashed vectors are used by default. To build an index with OpenAI embeddings:

```bash
OPENAI_API_KEY=... npx semantic-code-search-cli index --embeddings openai
```
